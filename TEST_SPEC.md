...
| H5 application architecture | Validate `APP_H5_ARCHITECTURE_SPEC.md`: `sdkwork-<application-code>-h5-*`, `sdkwork-<application-code>-h5-console-*`, and `sdkwork-<application-code>-h5-admin-*` package names, shared H5/Capacitor renderer, per-platform iOS/Android profiles inside one Capacitor host package, typed host adapters with their platform implementations, SDK/IAM boundaries, mobile config, and per-platform release metadata |
...
| Workspace path portability | Validate `DEPENDENCY_MANAGEMENT_SPEC.md` §1/§7/§9 with `tools/check-workspace-path-portability.mjs --workspace <checkout-root>` (zero findings): version-controlled source, configuration, and documentation contain no absolute path naming the relocatable workspace root (`WORKSPACE-ABS`), no `sdkwork-<name>` checkout that resolves through the consuming workspace mechanism (`SIBLING-REPO-ABS`), and no machine-rooted path outside the workspace (`MACHINE-ABS` — the last is on by default, because a hardcoded home directory, toolchain root, or installer cache is what breaks when the tree is built on another machine or OS; `--workspace-only` narrows the run to the two workspace rules); scan scope is `PORTABILITY_SPEC.md` §5.1 and the four reported exemption scopes are line `WORKSPACE-PATH:allow`, test-file `WORKSPACE-PATH:allow-fixture`, test-scoped-directory `.workspace-path-fixture`, and in-file `#[cfg(test)]` block `WORKSPACE-PATH:allow-fixture-block` per §5.2, with the exemption count and the exempt-file list printed on every run so no exemption is silent; the gate's own behaviour is pinned by `tools/check-workspace-path-portability.test.mjs` |
...
- Desktop/tablet-enabled PC roots `MUST` run the native host verification required by `DESKTOP_APP_ARCHITECTURE_SPEC.md`, including one host package per shipped architecture (`sdkwork-<application-code>-pc-tauri`, `-pc-electron`, `-pc-capacitor`), the absence of a second architecture's scaffold, dependencies, or config inside any host package, adapter ownership outside `pc-core/src/host/`, Tauri `devUrl`/`frontendDist`, Electron security baseline and preload allowlist, Capacitor provider pinning and plugin allow-list, host adapter contract parity, web fallback, and iPadOS/Android tablet packaging when enabled.
...
- Capacitor platform profile tests `MUST` prove every shipped platform has its native subtree, its `config/host/native/<platform>/` profile, its permission declaration, its adapter implementations, and its build command, and that an unshipped platform has none of them.
- Capacitor identity tests `MUST` fail when `app.identifiers.bundleId` or `app.identifiers.packageName`, the `capacitor.config.ts` `appId`, the Xcode bundle identifier, and the Gradle `applicationId` disagree.
- Capacitor adapter parity tests `MUST` typecheck each platform adapter implementation against the shared adapter interface and prove a browser fallback exists for every adapter in the host adapter catalog.
- Capacitor toolchain floor tests `MUST` prove the iOS deployment target and the Android `minSdkVersion`, `compileSdkVersion`, and `targetSdkVersion` match the platform contract for the declared Capacitor major.
- Capacitor generated-output tests `MUST` prove `ios/App/CapApp-SPM/Package.swift` and `ios/App/debug.xcconfig` are unmodified, that no keystore, provisioning profile, certificate, or project-key `google-services.json` is committed, and that no platform-split Capacitor host package exists.
...
- Capacitor host config tests `MUST` prove host config owns only bundle/package ids, schemes, app links/universal links, permissions, capabilities, icons, signing references, and store metadata, split into the shared Capacitor profile plus one profile per shipped platform.
...
- Harmony release preflight tests `MUST` validate Harmony direct or store package metadata, `HAP`/`HARMONY_APP` package format declarations, signing references, icons, screenshots, checksums/SBOM/provenance, and secret absence.

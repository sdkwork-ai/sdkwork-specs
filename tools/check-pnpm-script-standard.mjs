...
export const BROWSER_BUILD_ENV_ALIASES = new Set(['dev', 'test', 'staging', 'demo', 'prod']);
...
export const DESKTOP_HOST_FAMILY_AXES = new Set([
  'electron',
  'capacitor',
  'tauri',
  'standalone',
...
// PNPM_SCRIPT_SPEC.md section 4.1.2: the mobile family mirrors the desktop
// family with a platform axis instead of a host axis. Mobile development is
// profile-paired but never database-paired, so the database aliases are absent
// from this axis set on purpose: `dev:capacitor-ios:sqlite` is an axis mistake,
// not a client-local store validation.
export const MOBILE_HOST_FAMILY_ACTIONS = new Set([
  'dev',
  'build',
  'check',
  'test',
...
export const MOBILE_HOST_FAMILY_AXES = new Set([
  'ios',
  'ipados',
  'android',
  'standalone',
...
const HOST_FAMILIES = [
  { prefix: 'desktop', actions: DESKTOP_HOST_FAMILY_ACTIONS, axes: DESKTOP_HOST_FAMILY_AXES },
  { prefix: 'mobile', actions: MOBILE_HOST_FAMILY_ACTIONS, axes: MOBILE_HOST_FAMILY_AXES },
];

function isHostFamilyCommand(scriptName) {
  const parts = scriptName.split(':');
  const family = HOST_FAMILIES.find((entry) => entry.prefix === parts[0]);
  if (!family || parts.length < 2) return false;
  if (!family.actions.has(parts[1])) return false;
  return parts.slice(2).every((part) => family.axes.has(part));
}
const isDesktopHostFamilyCommand = isHostFamilyCommand;
const HOST_FAMILY_PREFIXES = new Set(HOST_FAMILIES.map((entry) => entry.prefix));

// PNPM_SCRIPT_SPEC.md section 4.5: a tool namespace selects an operation, not a
// runtime, so its second segment must be an action. Deployment profiles,
// runtime targets, and database aliases are the ambiguous values that leave
// `app-store:standalone:seed` unattributable to a runtime. Environment aliases
// and quality tiers are excluded from this set on purpose: `docs:dev`,
// `docs:check`, `docs:debug`, and `sbom:check` are legitimate actions whose
// token collides with an axis vocabulary, and rejecting them would gate real
// scripts with no replacement.
export const TOOL_NAMESPACES = new Set([
  'nginx',
  'docs',
  'perf',
  'migrate',
...
const TOOL_NAMESPACE_FORBIDDEN_SECOND_SEGMENTS = new Set([
  ...DEPLOYMENT_PROFILES,
  ...RUNTIME_TARGETS,
  ...DATABASE_ALIASES,
]);
...
const IGNORED_DIRS = new Set([
  ...
  '.pnpm',
  '.vite',
  // Native host projects are generated by the host toolchain rather than
  // authored here: the Tauri CLI owns `src-tauri/`, the approved Capacitor
  // Electron platform provider scaffolds `electron/`, `npx cap add ios|android`
  // generates `ios/` and `android/`, and Xcode/Gradle write their own caches.
  // Their `package.json` files carry upstream-tool script names that no SDKWork
  // author can fix, so auditing them reports a violation against a generated
  // file nobody can edit. Hand-written wrappers stay in scope at their owning
  // root; PNPM_SCRIPT_SPEC.md section 10 records the same boundary.
  'src-tauri',
  'electron',
  'ios',
  'android',
...
/**
 * PNPM_SCRIPT_SPEC.md section 4.5. The namespace-level contract: a tool
 * namespace is action-first, so its second segment is not a runtime selector.
 */
function pushToolNamespaceIssues(scriptName, issues, prefix = '') {
  const parts = scriptName.split(':');
  if (!TOOL_NAMESPACES.has(parts[0]) || parts.length < 2) return;
  if (!TOOL_NAMESPACE_FORBIDDEN_SECOND_SEGMENTS.has(parts[1])) return;
  issues.push(
    `${prefix}${scriptName}: tool namespaces are action-first; "${parts[1]}" is a deployment profile, runtime target, or database alias, not an action`,
  );
}

/**
 * PNPM_SCRIPT_SPEC.md section 4: a release-lane helper name must not begin with
 * a lifecycle phase followed by "-", because automation can then no longer tell
 * the phase from the helper: `release:sign-installers` reads as either phase
 * `sign` or helper `sign-installers`. Free-form helpers that do not shadow a
 * phase, such as `release:assert-ready`, stay valid.
 */
function pushReleasePhaseModifierIssues(scriptName, issues, prefix = '') {
  const parts = scriptName.split(':');
  const namespace = parts[0];
  if (namespace !== 'release' && namespace !== 'deploy') return;
  if (parts.length < 2) return;
  const phases = namespace === 'release' ? RELEASE_PHASES : DEPLOY_PHASES;
  const second = parts[1];
  if (phases.has(second)) return;
  for (const phase of phases) {
    if (!second.startsWith(`${phase}-`)) continue;
    issues.push(
      `${prefix}${scriptName}: "${second}" starts with the lifecycle phase "${phase}" plus a modifier; use ${namespace}:${phase}:<runtimeTarget|detail> so the phase stays identifiable`,
    );
    return;
  }
}
...
/**
 * PNPM_SCRIPT_SPEC.md section 4.1: a host-family prefix owns its own message.
 * `desktop` and `mobile` are families, not runtime-target aliases, so the
 * action-first rewrite hint would tell the reader to rename `desktop:foo` to
 * `foo:desktop`, which is not a valid script either. `pushHostFamilyIssues`
 * names the allowed actions and axes instead.
 */
function pushActionFirstRuntimeTargetIssues(scriptName, issues, prefix = '') {
  if (isHostFamilyCommand(scriptName)) return;
  const parts = scriptName.split(':');
  if (HOST_FAMILY_PREFIXES.has(parts[0])) return;
...
/**
 * PNPM_SCRIPT_SPEC.md section 4.1. A host family is a closed grammar: the second
 * segment is one of its actions and every segment after that is one of its
 * axes. Without this check a name such as `mobile:deploy` would pass on the
 * allowed first segment alone while selecting no host at all.
 */
function pushHostFamilyIssues(scriptName, issues, prefix = '') {
  if (isHostFamilyCommand(scriptName)) return;
  const parts = scriptName.split(':');
  const family = HOST_FAMILIES.find((entry) => entry.prefix === parts[0]);
  if (!family) return;
  const actions = [...family.actions].join(', ');
  if (parts.length < 2) {
    issues.push(
      `${prefix}${scriptName}: a host family prefix must be followed by an action (${actions})`,
    );
    return;
  }
  if (!family.actions.has(parts[1])) {
    issues.push(
      `${prefix}${scriptName}: "${parts[1]}" is not a ${parts[0]} host family action; use one of ${actions}`,
    );
    return;
  }
  const invalidAxis = parts.slice(2).find((part) => !family.axes.has(part));
  if (invalidAxis) {
    issues.push(
      `${prefix}${scriptName}: "${invalidAxis}" is not a ${parts[0]} host family axis`,
    );
  }
}
...
function pushRuntimeTargetAliasIssues(scriptName, issues, prefix = '') {
  ...
  for (const [index, part] of parts.entries()) {
    ...
    if (parts[0] === 'dev' && parts[1] === 'desktop' && (part === 'electron' || part === 'tauri' || part === 'capacitor')) {
      continue;
    }
...
function pushDevAxisIssues(scriptName, issues, prefix = '') {
  ...
  for (const part of parts.slice(1)) {
    ...
    if (parts[1] === 'desktop' && (part === 'electron' || part === 'tauri' || part === 'capacitor')) {
      continue;
    }
...
function pushCommandNameIssues(
  ...
  pushHostFamilyIssues(scriptName, issues, prefix);
  ...
  pushToolNamespaceIssues(scriptName, issues, prefix);
  pushReleasePhaseModifierIssues(scriptName, issues, prefix);

...
| Workspace path portability | Validate `DEPENDENCY_MANAGEMENT_SPEC.md` §1/§7/§9 with `tools/check-workspace-path-portability.mjs --workspace <checkout-root>` (zero findings): version-controlled source, configuration, and documentation contain no absolute path naming the relocatable workspace root (`WORKSPACE-ABS`) or a `sdkwork-<name>` checkout that resolves through the consuming workspace mechanism (`SIBLING-REPO-ABS`); scan scope is `PORTABILITY_SPEC.md` §5.1 and the reported exemptions are line-scope `WORKSPACE-PATH:allow` and test-file `WORKSPACE-PATH:allow-fixture` per §5.2, both printed on every run so no exemption is silent |

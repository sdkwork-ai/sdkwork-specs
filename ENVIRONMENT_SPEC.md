...
| `SDKWORK_<APPLICATION_CODE>_REGION_CODE` | private | SHOULD | Deployment/market region code per `REGION_SPEC.md` sections 4 and 8: lowercase ASCII `^[a-z][a-z0-9_]*$`, at most 64 characters, default `global`. Every installable/startable application MUST be able to resolve its default region at startup; installers and deployment manifests SHOULD declare this key. The region is orthogonal to the deployment profile and environment and MUST NOT become a profile-id segment. |
| `SDKWORK_REGION_CODE` | private | MAY | Shared process-wide default region code resolved by framework bootstraps (such as `sdkwork-web-framework`) after probing `SDKWORK_<APPLICATION_CODE>_REGION_CODE` keys. Same format and default as the application key above. |
...
- Container deployments in the `development` environment MUST resolve the same physical host PostgreSQL instance and the identical workspace identity (database, schema, username, password) as the checked-in `.env.postgres` profile; only the reachable host differs (`127.0.0.1` from host processes, `host.docker.internal` from containers). `DEPLOYMENT_SPEC.md` §6.1 owns the Docker external host-system dependency standard.

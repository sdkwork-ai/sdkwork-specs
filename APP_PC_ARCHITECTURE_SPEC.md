...
| Host package naming | `node <sdkwork-specs>/tools/check-client-host-packages.mjs --root .` proves every native host package is architecture-explicit, is owned by the matching client root, keeps exactly one package per architecture, and carries the manifest of its own ecosystem: `package.json` for a PC, H5, or mini program root, and `oh-package.json5`, `pubspec.yaml`, `build.gradle.kts`, or `Package.swift` for a Harmony, Flutter, native Android, or native iOS root (`HARMONY_APP_MOBILE_ARCHITECTURE_SPEC.md` section 4). The retired `-pc-desktop` alias, and an inert `package.json` inside a native host package, are reported as migration debt and fail the gate under `--strict`. |

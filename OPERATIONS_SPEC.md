...
Automated audit: `node ../sdkwork-specs/tools/check-operations-conformance.mjs --root <module-root>`
covers the structural items below (bin/ entry set, bootstrap ordering, bundle
release channel, compose log rotation, runbooks, module wiring). Run it in CI
and on every `bin/` or bundle change; a `FAIL` blocks the release train.

- [ ] All nine lifecycle stages are reachable from `bin/` without reading module scripts.
- [ ] The bundle ships `release.sh` implementing §1.2 (health gate, auto-rollback, ledger, lock, digest identity).
- [ ] Every compose file that starts a service declares `logging:` with rotation.

...
| Code style or naming change | `CODE_STYLE_SPEC.md`, `NAMING_SPEC.md` | Only the touched language/framework spec: `RUST_CODE_SPEC.md`, `JAVA_CODE_SPEC.md`, `TYPESCRIPT_CODE_SPEC.md`, or `FRONTEND_CODE_SPEC.md`; `PNPM_SCRIPT_SPEC.md` for package command standardization, package scripts, root lifecycle commands, host-family commands, or the pnpm runtime and dependency-install policy |
...
| Repair a defect, restore content lost to a revert, or resolve a branch or remote divergence | `ROLLBACK_RESTRICTION_SPEC.md`, `SOUL.md` §5.1, `CODE_STYLE_SPEC.md` §8.1 | `DESTRUCTIVE_OPERATION_SPEC.md` when the recovery deletes or restores files, `CODE_REVIEW_SPEC.md`, `GOVERNANCE_SPEC.md` §3 when a rollback exception is requested; verify with `tools/sync-agent-rollback-restriction-standard.mjs --check` |
...
| Upload, file storage, download, or app-upload capability | `DRIVE_SPEC.md` | `APP_SDK_INTEGRATION_SPEC.md`, `FRONTEND_SPEC.md`, `API_SPEC.md`, `MEDIA_RESOURCE_SPEC.md` when a media representation is touched, `APP_MANIFEST_SPEC.md` for application identity, `TEST_SPEC.md`; author the application upload declaration per `DRIVE_SPEC.md` §18 |
...
Defect repair and recovery follow `ROLLBACK_RESTRICTION_SPEC.md` and `CODE_STYLE_SPEC.md` §8.1: errors are fixed forward, version-control state is never moved to make a failure disappear (`git reset`, `git checkout -f`, `git switch -f`, `git restore` toward an earlier state, reflex `git revert`, stash discard, ref deletion, `git rebase`, `git commit --amend` on pushed history, or a force push), recovery is additive with an explicit enumerated pathspec, and discarding work requires a separate explicit human instruction that is recorded in the commit message. Every repository `AGENTS.md` carries the tool-owned `SDKWORK-ROLLBACK-RESTRICTION-STANDARD` block; refresh it with `node ./tools/sync-agent-rollback-restriction-standard.mjs --workspace .. --apply`.
...
API work loads `API_SPEC.md` and its validators. List/search work loads `PAGINATION_SPEC.md` and `check-pagination.mjs`. Source configuration work loads `SOURCE_CONFIG_SPEC.md` and `check-source-config-standard.mjs`. Web server configuration work loads `SDKWORK_WEBSERVER_SPEC.md` and `check-webserver-toml-standard.mjs`. Upload, file-storage, download, Drive Uploader, or app-upload work loads `DRIVE_SPEC.md`; an application upload declaration is authored at `<application-root>/specs/upload.declaration.json` per `DRIVE_SPEC.md` section 18, and `MEDIA_RESOURCE_SPEC.md` is added when a media representation contract is touched. Rollback, restore, and defect-repair work loads `ROLLBACK_RESTRICTION_SPEC.md` and `sync-agent-rollback-restriction-standard.mjs`. Client application root work loads `APP_CLIENT_ARCHITECTURE_ALIGNMENT_SPEC.md` plus the matching root architecture standard, and loads `DESKTOP_APP_ARCHITECTURE_SPEC.md` for any Tauri, Electron, or Capacitor desktop host or `APP_H5_ARCHITECTURE_SPEC.md` §8 for the one H5 Capacitor mobile host. Desktop host package naming is owned by `NAMING_SPEC.md` §3 and the `desktop:*` command family by `PNPM_SCRIPT_SPEC.md` §4.1.1. Link these authorities instead of copying their normative bodies into `AGENTS.md`.

## Rollback Restriction And Fix-Forward

Defect repair, restore, and branch or remote divergence work loads `ROLLBACK_RESTRICTION_SPEC.md`. Errors are fixed forward: moving version-control state is never the remedy for a build failure, type error, lint failure, test failure, merge conflict, regression, bad refactor, or unclear diff. Before completion, run:

```bash
node tools/sync-agent-rollback-restriction-standard.mjs --root . --check
node tools/sync-agent-rollback-restriction-standard.mjs --workspace .. --check
```

The canonical rules arrive as the tool-owned `SDKWORK-ROLLBACK-RESTRICTION-STANDARD` managed block at the end of this file and of every repository `AGENTS.md`; this entrypoint is only the task trigger and verification route.

## Human Review Rules

Human review is required for new root standards, breaking standard changes, security exceptions, naming migrations, rollback exceptions, and changes that affect all repositories or application roots.
...
Authority: `./ROLLBACK_RESTRICTION_SPEC.md`.
...
Authority: `./REPOSITORY_BASELINE_SPEC.md` section 1.
...
- A working tree found detached, or on a branch other than `main`, with work in it is a STOP, not a
  ...
  discarding work are both governed by `./DESTRUCTIVE_OPERATION_SPEC.md` and
  `./ROLLBACK_RESTRICTION_SPEC.md`.

Verification (from the repository root):

```bash
node ./tools/audit-repository-baseline.mjs --root . --only branch-main
node ./tools/sync-agent-main-branch-standard.mjs --root . --check
```

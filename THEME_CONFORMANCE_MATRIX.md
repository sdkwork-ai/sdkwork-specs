...
| `sdkwork-mcp` (MCP 注册/市场/管理面) | aligned (2026-09-17) — console + admin surfaces are **embedded-only** inside the Web Server host, which owns mode; the standalone `sdkwork-mcp-pc` app mounts no provider and is light-only by construction (documented) | partial — paired `dark:` utilities on the zinc ramp, deliberately matched to the host's `--sdk-color-surface-panel*` (#18181b/#27272a); form controls additionally consume the host token contract `.skills-console-field*` / `.skills-console-primary` / `.sdkwork-surface-modal-confirm--danger` | `none` — stage target after standalone settings work | **aligned (fixed 2026-09-17)** — entry had no `@custom-variant dark` while sources use `dark:` (F4, P0); `@variant dark (&:is(.dark *))` now bound | OK (383) | Host-live surfaces (console/admin) are AA-clean in light (worst 4.76) and bounded at 3.38:1 in dark by the host's own `--webserver-color-on-accent` token (`webserver src/index.css:79-83`), not by this module. Fixing the reset `button,input,select,textarea{color:inherit}` (unlayered ⇒ outranks all utility-layer text colors on `<button>`) is a host-repo change with brand-wide blast radius — tracked separately. |
...
| `sdkwork-mcp` H5 mobile shell (`apps/sdkwork-mcp-h5`) | **dark-only by construction**, not a dual-mode surface: `src/index.css` pins `:root { color-scheme: dark }` with body `#141414`/`#f3f4f6`, the app mounts no theme provider (no `SdkworkThemeProvider`, no `data-sdk-color-mode`, no `.dark` writes), and every surface is authored dark-native (`text-white`, `text-gray-400`, `bg-[#1f1f1f]`). Its `text-gray-500` labels are ~5.6:1 on `#1f1f1f` — correct as written. F5 pairing is therefore not applicable; an automated F5 scan flags 3 such labels and must be read as a false positive here. Revisit only if a light mode is ever introduced. |

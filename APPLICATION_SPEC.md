...
| uni-app cross-platform app root | `UNIAPP_APP_ARCHITECTURE_SPEC.md`; `MP_*` delivery packaging also applies `MINI_PROGRAM_APP_ARCHITECTURE_SPEC.md`; uni-app admin packages also apply `BACKEND_UI_SPEC.md` | `apps/sdkwork-<application-code>-uniapp/packages/sdkwork-<application-code>-uniapp-*`, `sdkwork-<application-code>-uniapp-console-*`, `sdkwork-<application-code>-uniapp-admin-*` |
| Unity app root | `UNITY_APP_ARCHITECTURE_SPEC.md` | `apps/sdkwork-<application-code>-unity/Packages/com.sdkwork.<application-code>-unity-*`, `com.sdkwork.<application-code>-unity-console-*`, `com.sdkwork.<application-code>-unity-admin-*` |
| Static web app root | `APP_STATIC_WEB_ARCHITECTURE_SPEC.md` | `apps/sdkwork-<application-code>-static-web/packages/sdkwork-<application-code>-static-web-*` page-group packages |
...
- uni-app cross-platform roots `MUST` follow `UNIAPP_APP_ARCHITECTURE_SPEC.md`. uni-app packages use `sdkwork-<application-code>-uniapp-*`, `sdkwork-<application-code>-uniapp-console-*`, or `sdkwork-<application-code>-uniapp-admin-*`; `pages.json` output is a route-contribution projection, and conditional compilation stays in the host package and bootstrap.
- Unity roots `MUST` follow `UNITY_APP_ARCHITECTURE_SPEC.md`. Unity packages use UPM ids `com.sdkwork.<application-code>-unity-<role>` with Assembly-Definition-enforced dependency direction; the root bootstrap scene and `Assets/Scripts/Bootstrap/` stay thin, and tokens persist only through secure-storage host adapters.
- Static web roots `MUST` follow `APP_STATIC_WEB_ARCHITECTURE_SPEC.md`. Static web packages use `sdkwork-<application-code>-static-web-core` and `sdkwork-<application-code>-static-web-<page-group>` page groups; the root owns content only and must not implement login/session behavior.
...
- H5 mobile, Flutter mobile, mini program, uni-app, Unity, static web, native Android, native iOS, and native HarmonyOS app shells `MUST` follow `APP_CLIENT_ARCHITECTURE_ALIGNMENT_SPEC.md` and their root architecture standards for package taxonomy, route identity, host adapters, and renderer/platform packaging boundaries.
...
| uni-app app | TypeScript app SDKs adapted for uni-app runtime and uni-app host adapters | appbase uni-app wrapper or approved appbase IAM runtime adapter with one global TokenManager equivalent |
| Unity app | C# app SDKs and Unity host adapters | appbase Unity wrapper or approved appbase C# adapter with one global token-manager equivalent backed by secure storage |
| Static web app | generated public/open-api SDK clients through core runtime config, or build-time content only | no login/session; declared public surfaces only |
